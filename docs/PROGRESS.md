...
| hardening (this cycle) | Lattice-inspired: AGENTS.md, docs/TRAPS.md, seam spec (reducer→ps_data→lobby), Playwright visual harness + visual-gate workflow, this file | green (`pnpm test`, `typecheck`, `verify:visual` ×2) | the first harness run found and fixed trap #10 (gauge fallback deadlock); two bug-injections confirmed the gate bites | next story chapter |
